...
describe("fixture monitoring provider", () => {
  ...
  it("exposes only safe authenticated session identity and typed authentication failures", async () => {
    ...
    const live = createLiveMonitoringProvider({
      fetchImpl: () => Promise.resolve(new Response(JSON.stringify({
        apiVersion: 1,
        authenticated: true,
        user: { id: "access:user", displayName: "Operator", role: "operator" },
        expiresAt: "2026-08-17T00:00:00.000Z"
      }), { status: 200 })),
      timeoutMs: 1000
    });
    await expect(live.getSession()).resolves.toMatchObject({ user: { id: "access:user", role: "operator" } });
  ...
  it("loads persistent incidents and sends strict commands without a browser-selected actor", async () => {
    ...
    const list = {
      ...
      operator: { id: "access:lab-operator", displayName: "Lab Operator", role: "operator", identityMode: "cloudflare-access" }, incidents: [incident]
    } as const;
